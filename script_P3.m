...
model_path = 'P3.slx';
save_folder_path = "output-P3\img\";
save_folder_path_final = "output-P3\";

fig = figure('Position', [0,0,600,600]);
fig2 = figure('Position', [0 0 710 155]);
...
velR_ref_all = cell(1, size(Data_input, 1));
velL_ref_all = cell(1, size(Data_input, 1));
...
last_data = struct('velocity', [], 'angular_speed', [], 'wheel_distance_left', [], 'wheel_distance_right', [], 'distance', [], 'angular_position', [], 'wheel_velocity_right', [], 'wheel_velocity_left', [], 'wheel_velocity_right_ref', [], 'wheel_velocity_left_ref', []');
...
for i = 1:size(Data_input, 1)
    ...
    % get data output
    [wheel_velocity_left, wheel_velocity_right, velocity, wheel_distance_left, wheel_distance_right, distance, angular_position, angular_speed, x_position, y_position, wheel_velocity_left_ref, wheel_velocity_right_ref] = get_data(simRes);
    
    % accumulate data
    time_all{i} = velocity.Time;
    velR_all{i} = wheel_velocity_right.Data;
    velL_all{i} = wheel_velocity_left.Data;
    velR_ref_all{i} = wheel_velocity_right_ref.Data;
    velL_ref_all{i} = wheel_velocity_left_ref.Data;
    ...
    last_data.wheel_velocity_right_ref(i) = wheel_velocity_right_ref.Data(end);
    last_data.wheel_velocity_left_ref(i) = wheel_velocity_left_ref.Data(end);
...
plot_all_data(fig, "Right Wheel Velocity Reference", "Time (s)", "Velocity (m/s)", time_all, velR_ref_all, velR_legend_str, save_folder_path_final, "right_wheel_velocity_ref");
plot_all_data(fig, "Left Wheel Velocity Reference", "Time (s)", "Velocity (m/s)", time_all, velL_ref_all, velR_legend_str, save_folder_path_final, "left_wheel_velocity_ref");
...
right_wheel_velocity_ref_image = append(save_folder_path_final, "right_wheel_velocity_ref.png");
left_wheel_velocity_ref_image = append(save_folder_path_final, "left_wheel_velocity_ref.png");
VrefR_image = imread(right_wheel_velocity_ref_image);
VrefL_image = imread(left_wheel_velocity_ref_image);
combinedImage = [VrefR_image VrefL_image];
imwrite(combinedImage, append(save_folder_path_final, "velocity_ref.png"));
...
function plot_table(fig, data, save_folder_path, filename)
    ...
    data = [
        data.wheel_distance_left; data.wheel_distance_right; data.wheel_velocity_left_ref; data.wheel_velocity_right_ref; data.angular_speed; data.wheel_velocity_left; data.wheel_velocity_right; data.distance; data.angular_position
    ];
    data = transpose(data);

    uitable('Data', data, 'ColumnName', {'SL (m)', 'SR (m)', 'VrefL (m/s)', 'VrefR (m/s)', 'ω (deg/s)', 'Vl (m/s)', 'Vr (m/s)', 'S (m)', 'θ (deg)'}, 'Position', [0 0 fig.Position(3:4)]);
...
function [wheel_velocity_left, wheel_velocity_right, velocity, wheel_distance_left, wheel_distance_right, distance, angular_position, angular_speed, x_position, y_position, wheel_velocity_left_ref, wheel_velocity_right_ref] = get_data(simRes)
    ...
    x_position              = simRes.X;
    y_position              = simRes.Y;
    wheel_velocity_left_ref = simRes.VrefL;
    wheel_velocity_right_ref = simRes.VrefR;     
end