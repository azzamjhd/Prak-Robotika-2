% Data input
Data_input = [[1, pi/2]; [2, pi/3]; [3, pi/6]; [1, pi/6]; [2, pi/3]; [3, pi/2]];

model_path = 'P1_T1.slx';
save_folder_path = "output-T1\img\";
save_folder_path_final = "output-T1\";
...
function plot_table(fig, data, save_folder_path, filename)
    ...
    data = [
        data.velocity; data.angular_speed; data.wheel_vel_left; data.wheel_vel_right; data.wheel_distance_left; data.wheel_distance_right; data.distance; data.angular_position
    ];
    data = transpose(data);

    uitable('Data', data, 'ColumnName', {'V (m/s)', 'ω (deg/s)', 'vl (m/s)', 'vr (m/s)', 'Sl (m)', 'Sr (m)', 'S (m)', 'θ (deg)'}, 'Position', [0 0 fig.Position(3:4)]);
