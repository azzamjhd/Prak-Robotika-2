...
fig2 = figure('Position', [0 0 490 160]);
...
last_data = struct('velocity', [], 'angular_speed', [], 'wheel_distance_left', [], 'wheel_distance_right', [], 'distance', [], 'angular_position', []);
...
for i = 1:size(Data_input, 1)
    ...
    % Plot last data points on table
    % plot_table(fig2, last_points, save_folder_path_final, append(num2str(i), ". table"));
...
function plot_table(fig, data, save_folder_path, filename)
    ...
    data = [
        data.velocity; data.angular_speed; data.wheel_distance_left; data.wheel_distance_right; data.distance; data.angular_position
    ];
    data = transpose(data);

    uitable('Data', data, 'ColumnName', {'V (m/s)', 'ω (deg/s)', 'Sl (m)', 'Sr (m)', 'S (m)', 'θ (deg)'}, 'Position', [0 0 fig.Position(3:4)]);
